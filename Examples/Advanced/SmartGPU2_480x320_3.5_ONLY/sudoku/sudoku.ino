...
void drawScreen() {
  ...
  if (selectedScreen == SCREEN_OPTIONS) {
    ...
    lcd.string(PENCIL_ERROR_LEFT + 2, PENCIL_ERROR_TOP - 22, MAX_X_LANDSCAPE, MAX_Y_LANDSCAPE, "Show :", &nb);
    ...
    lcd.string(PENCIL_ERROR_LEFT + CHECK_WIDTH + CHECK_LABEL_SPACING, PENCIL_ERROR_TOP + 2, MAX_X_LANDSCAPE, MAX_Y_LANDSCAPE, "Pencil Errors", &nb);
    ...
    lcd.string(PEN_ERROR_LEFT + CHECK_WIDTH + CHECK_LABEL_SPACING, PEN_ERROR_TOP + 2, MAX_X_LANDSCAPE, MAX_Y_LANDSCAPE, "Pen Errors", &nb);
    ...
    lcd.string(COMMAND_SAVE_LEFT + 22, COMMAND_SAVE_TOP + 9, MAX_X_LANDSCAPE, MAX_Y_LANDSCAPE, "Save Game", &nb);
    ...
    lcd.string(COMMAND_RESTORE_LEFT + 9, COMMAND_RESTORE_TOP + 9, MAX_X_LANDSCAPE, MAX_Y_LANDSCAPE, "Restore Game", &nb);
    ...
    lcd.string(COMMAND_PLAY_LEFT + 47, COMMAND_PLAY_TOP + 9, MAX_X_LANDSCAPE, MAX_Y_LANDSCAPE, "Play !", &nb);
    ...
    lcd.setTextColour(SG_WHITE);    
    lcd.setTextSize(SG_FONT1);
    lcd.string(COMMAND_MENU_LEFT + 32, COMMAND_MENU_TOP + 9, MAX_X_LANDSCAPE, MAX_Y_LANDSCAPE, "Menu", &nb);
...
void drawTab() {
  ...
  lcd.string(27, 6, MAX_X_LANDSCAPE, MAX_Y_LANDSCAPE, "Easy", &nb);
  lcd.setTextColour(selectedTab == TAB_MEDIUM ? SG_WHITE : GREY5);    
  lcd.string(108, 6, MAX_X_LANDSCAPE, MAX_Y_LANDSCAPE, "Medium", &nb);
  lcd.setTextColour(selectedTab == TAB_HARD ? SG_WHITE : GREY5);    
  lcd.string(204, 6, MAX_X_LANDSCAPE, MAX_Y_LANDSCAPE, "Hard", &nb);
  ...
  lcd.string(TAB_PAGE_TEXT_LEFT, TAB_PAGE_TEXT_TOP, MAX_X_LANDSCAPE, MAX_Y_LANDSCAPE, buf, &nb);
...
void drawCell(char x, char y, boolean highlight) {
  ...
  if (solution[x][y] > 0) {
    ...
    lcd.string((x * CELL_SPACING) + 11, (y * CELL_SPACING) + 6, MAX_X_LANDSCAPE, MAX_Y_LANDSCAPE, buf, &nb);
      ...
      for (byte x1=1; x1<=3; x1++) {
        ...
        for (byte y1=0; y1<=2; y1++) {
          ...
          if (board[x][y][(y1*3)+x1] > 0) {
            ...
            lcd.string((x * CELL_SPACING) - 4 + (x1 * 9), (y * CELL_SPACING) + 1 + (y1 * 10), MAX_X_LANDSCAPE, MAX_Y_LANDSCAPE, buf, &nb);
...
void drawPencilBoard(char selectedX, char selectedY, boolean highlight) {
  ...
  for (byte y1=0; y1<=2; y1++) {
    ...
    for (byte x1=1; x1<=3; x1++) {
      ...
      itoa(i, buf, 10);
      lcd.string(((x1 - 1) * CELL_SPACING) + PENCIL_LEFT - 4 + (x1 * 9), (y1 * CELL_SPACING) + PENCIL_TOP + 1 + (y1 * 10), MAX_X_LANDSCAPE, MAX_Y_LANDSCAPE, buf, &nb);
      i++;
  ...
  lcd.string(255, PENCIL_LABEL_LEFT + 6, MAX_X_PORTRAIT, MAX_Y_PORTRAIT, "Pencil", &nb);
...
void drawPenBoard(char selectedX, char selectedY, boolean highlight) {
  ...
  for (byte y=0; y<=2; y++) {
    ...
    for (byte x=1; x<=3; x++) {
      ...
      itoa((y*3)+x, buf, 10);
      lcd.string(((x - 1) * CELL_SPACING) + 11 + PEN_LEFT, (y * CELL_SPACING) + 6 + PEN_TOP, MAX_X_LANDSCAPE, MAX_Y_LANDSCAPE, buf, &nb);
  ...
  lcd.orientation(SG_PORTRAIT_LOW);
  lcd.string(PEN_LABEL_TOP + 18, PEN_LABEL_LEFT + 6, MAX_X_PORTRAIT, MAX_Y_PORTRAIT, "Pen", &nb);
  lcd.orientation(SG_LANDSCAPE_LEFT);
...
void drawCompleteMessage() {
  SG_NUMBEROFBYTES nb;
  lcd.setTextSize(SG_FONT3);
  lcd.string(164, 164, MAX_X_LANDSCAPE, MAX_Y_LANDSCAPE, "Complete !", &nb);
...
void die(SG_FILERESULT response){ //if the response is different than OK, print and loop forever
  SG_NUMBEROFBYTES nb;
  if(response!=SG_F_OK){
    lcd.string(10,20,MAX_X_LANDSCAPE,MAX_Y_LANDSCAPE,"Error... forever loop @",&nb);
    while(1); 
  }
  ...
  do {
    ...
    if (bytesRead == RECORD_DESC_LENGTH) {
      ...
      if (i >= (selectedTab_Page * TAB_ENTRIES_PER_PAGE) && i < ((selectedTab_Page + 1) * TAB_ENTRIES_PER_PAGE)) {
        ...
        lcd.string(20, row + 8, MAX_X_LANDSCAPE, MAX_Y_LANDSCAPE, buffer, &nb);
        row+=GRID_ROW_HEIGHT;       
...
int showDialog(char caption[], char message[], int buttons) {
  ...
  lcd.string(DIALOG_LEFT + DIALOG_MESSAGE_LEFT, DIALOG_TOP + DIALOG_MESSAGE_TOP, DIALOG_LEFT + DIALOG_MESSAGE_LEFT + DIALOG_MESSAGE_WIDTH, MAX_Y_LANDSCAPE, message, &nb);
  ...
  if (buttons == DIALOG_BUTTONS_OKCANCEL) {
    ...
    lcd.string(DIALOG_LEFT + DIALOG_2BUTTONS_BUTTON1_LEFT + 34, DIALOG_TOP + DIALOG_BUTTON_TOP + 11, MAX_X_LANDSCAPE, MAX_Y_LANDSCAPE, "OK", &nb);
    lcd.string(DIALOG_LEFT + DIALOG_2BUTTONS_BUTTON2_LEFT + 18, DIALOG_TOP + DIALOG_BUTTON_TOP + 11, MAX_X_LANDSCAPE, MAX_Y_LANDSCAPE, "Cancel", &nb);
  ...
  if (buttons == DIALOG_BUTTONS_OKONLY) {
    ...
    lcd.string(DIALOG_LEFT + DIALOG_1BUTTONS_LEFT + 34, DIALOG_TOP + DIALOG_BUTTON_TOP + 11, MAX_X_LANDSCAPE, MAX_Y_LANDSCAPE, "OK", &nb);
