...
void getCurrentWallFromEEPROM(void){
  ...
  lcd.readEEPROMBuff(name,0,14,&nb);      //read 14 bytes of the EEPROM RAM buffer to name array, wallpapers must not exceed 14 chars in name lenght
...
void saveWallpaperToEEPROM(void){
  ...
  lcd.writeEEPROMBuff(name,0,sizeof(name),&nb);    //write all the contents of "name" array to EEPROM RAM Buffer
...
void media(){
  ...
  while(1){
    ...
    lcd.playWAVFile(name,&seconds);   //open audio if any, must be named the same as the video expept for the .extension
    while(1){
      ...
      if(point.y>265){                            //Touch on controls
        if(point.x<80){                           //touch on << icon
          ...
          if(state == SG_ENABLE){                 //if playing
            ...
            lcd.playWAVFile(name,&seconds);       //play file 
            lcd.drawRectangle(0,25,MAX_X_PORTRAIT,70,SG_BLACK,SG_FILL);   //erase previous name
            lcd.string(10,30,MAX_X_PORTRAIT,MAX_Y_PORTRAIT,name,&nb); //print new name
            pause=0; 
          }        
          delay(300);          
