...
void getCurrentWallFromEEPROM(void){
  ...
  lcd.readEEPROMBuff(nam,0,14,0);  //read 14 bytes of the EEPROM RAM buffer to name array, wallpapers must not exceed 14 chars in name lenght
  if(strstr(nam, 0x00) != 0x00){        //find a null character in name, if not find, add the 0x00 NULL character
    nam[0] = 0x00;
  }
}
...
void saveWallpaperToEEPROM(void){
  ...
  lcd.writeEEPROMBuff(nam,0,sizeof(nam),&nb);    //write all the contents of "name" array to EEPROM RAM Buffer
...
void drawHeader(char *text){
  ...
  lcd.string(2,0,MAX_X_PORTRAIT,20,"Vizic",&nb);             //draw Vizic string
  lcd.string((LCD_HEIGHT/2)-35,0,MAX_X_PORTRAIT,20,text,&nb); //draw string
...
void cutFileExtension(char *nam, char *ext){
  char *pch;
  pch=strstr(nam,ext);        //find the .ext extension to the name
...
void drawMainMenu(void){  
  ...
  strcat(aux,nam);                             //concatenate the currentWall name to "Wall/"
...
void backlight(void){
  ...
  lcd.setTextColour(SG_WHITE);
  lcd.setTextBackFill(SG_TRANS);
  lcd.string((MAX_X_PORTRAIT+1)/6,(MAX_Y_PORTRAIT+1)/4,MAX_X_PORTRAIT,MAX_Y_PORTRAIT,"Brightness Bar",&nb);
...
void photos(void){
  ...
  lcd.string((MAX_X_LANDSCAPE+1)/3,(MAX_Y_LANDSCAPE+1)/3,MAX_X_LANDSCAPE,MAX_Y_LANDSCAPE,"Photo Gallery",&nb);
  ...
  while(1){  
    lcd.SDFgetFileName(i,nam);                        //get the name of the pic file number i
    cutFileExtension(nam,".bmp");                     //cut to name the .bmp extension
    lcd.imageBMPSD(0,0,nam);                          //Load image from SD card, all images must be full screen so we load them from top left corner X:0,Y:0
    delay(200);                                        //A little delay to avoid fast image changing    
    lcd.string((MAX_X_LANDSCAPE+1)/4,MAX_Y_LANDSCAPE-20,MAX_X_LANDSCAPE,MAX_Y_LANDSCAPE,"<Tap center to Exit>",&nb); //Show text
...
void media(){
  SG_NUMBEROFBYTES nb;
  unsigned int i=0, row=0, vids=0, seconds;
  ...
  lcd.setTextBackFill(SG_TRANS);
  lcd.drawGradientRect(0,0,MAX_X_LANDSCAPE,MAX_Y_LANDSCAPE,SG_BLACK,SG_MAGENTA,SG_VERTICAL);
  lcd.string((MAX_X_LANDSCAPE+1)/3,(MAX_Y_LANDSCAPE+1)/3,MAX_X_LANDSCAPE,MAX_Y_LANDSCAPE,"Video Player",&nb);  
  ...
  while(1){
    lcd.drawGradientRect(0,0,MAX_X_LANDSCAPE,MAX_Y_LANDSCAPE,SG_BLACK,SG_MAGENTA,SG_VERTICAL);
    lcd.string(5,0,MAX_X_LANDSCAPE,MAX_Y_LANDSCAPE,"Available Videos:      (EXIT)",&nb);
    i=0; row=1; vids=0;
    while(1){                          //list names
      lcd.SDFgetFileName(i++,nam);    //get the name of the vid file number i
      if(nam[0]==0x00) break;         //if name is invalid, meand end of files 
      if(strstr(nam,".vid")!=0x00){   //if .vid extension is found in the file name: print      
        lcd.objButton(15,(row*30),MAX_X_LANDSCAPE-15,30+(row*30),SG_DESELECTED,nam);  row++;
        vids++;  
    ...
    while(row){ //get name of obtained row
      lcd.SDFgetFileName(i++,nam);    //get the name of the vid file number i  
      if(strstr(nam,".vid")!=0x00) row--;   //if .vid extension is found in the file name: decrease row variable
    }
    //Try to play video
    cutFileExtension(nam,".vid");    //cut to name the .vid extension  
    if(lcd.allocateVideoSD(nam,&videoData)!=SG_OK) continue; //try to allocate video, if fail, continue to top while(1)
    //up to here video is successfully allocated..
    currentFrame=0;  currentSecond=0; //reset variables
    lcd.playWAVFile(nam,&seconds);   //open audio if any, must be named the same as the video expept for the .extension
    while(1){
...
void music(){
  ...
  unsigned int songs=0, seconds=0;
  ...
  lcd.string((MAX_X_PORTRAIT+1)/4,(MAX_Y_PORTRAIT+1)/2,MAX_X_PORTRAIT,MAX_Y_PORTRAIT,"Music Player",&nb);
  delay(1000);
  lcd.erase();  
  drawHeader("Music:");
      ...
      if(point.y>265){                            //Touch on controls
        if(point.x<80){                           //touch on << icon
          ...
          if(state == SG_ENABLE){                 //if playing
            ...
            lcd.SDFgetFileName(currentSong,nam); //get the name of the song file number currentSong
            cutFileExtension(nam,".wav");        //cut to name the .wav extension
            lcd.playWAVFile(nam,&seconds);       //play file 
            lcd.drawRectangle(0,25,MAX_X_PORTRAIT,70,SG_BLACK,SG_FILL);   //erase previous name
            lcd.string(10,30,MAX_X_PORTRAIT,MAX_Y_PORTRAIT,nam,&nb); //print new name
            pause=0; 
          }        
          delay(300);          
...
void wallpaper(){
  ...
  lcd.setTextBackFill(SG_TRANS);
  lcd.drawGradientRect(0,0,MAX_X_PORTRAIT,MAX_Y_PORTRAIT,SG_BLACK,SG_MAGENTA,SG_VERTICAL);
  lcd.string((MAX_X_PORTRAIT+1)/4,(MAX_Y_PORTRAIT+1)/2,MAX_X_PORTRAIT,MAX_Y_PORTRAIT,"Wallpapers",&nb);
  delay(1000);
 
  while(1){  
    lcd.SDFgetFileName(i,nam);                        //get the name of the wall file number i
    cutFileExtension(nam,".bmp");                     //cut to name the .bmp extension
    lcd.imageBMPSD(0,0,nam);                          //Load image from SD card, all images are 240x320(full screen) so we load them from top left corner X:0,Y:0
    ...
    if(point.y < 60){                                  //if touch on "set as wall" button
      ...
      lcd.setTextSize(SG_FONT2);
      lcd.string(22,140,MAX_X_PORTRAIT,MAX_Y_PORTRAIT,"Saved on EEPROM",&nb);
