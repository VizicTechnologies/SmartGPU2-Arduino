...
void error(unsigned char response){ //if the response is different than OK, print and loop forever
  SG_NUMBEROFBYTES nb;
  if(response!=SG_F_OK){
    lcd.string(10,row,LCD_WIDTH-1,LCD_HEIGHT-1,"Error on microSD... forever loop@",&nb);
    while(1);  
  }
}
...
void loop() { //main loop 
    ...
    lcd.string(10,row,LCD_WIDTH-1,LCD_HEIGHT-1,"List dirs, files + print names demo!",&nb); row+=20;
    lcd.setTextSize(SG_FONT0);        
    lcd.string(10,row,LCD_WIDTH-1,LCD_HEIGHT-1,"List Dirs and Files...",&nb);             row+=15;
    res=lcd.SDFgetList(&dirs,&files);    //obtain dirs and files
    error(res);
    lcd.string(10,row,LCD_WIDTH-1,LCD_HEIGHT-1,"Dirs:",&nb);
    lcd.printNumber(40,row,dirs); //print the obtained directories
    lcd.string(70,row,LCD_WIDTH-1,LCD_HEIGHT-1,"Files:",&nb);    
    lcd.printNumber(105,row,files); //print the obtained files    
    row+=15;

    //print dir names
    lcd.string(10,row,LCD_WIDTH-1,LCD_HEIGHT-1,"Dir Names--------------------",&nb);             row+=12;
    for(i=0;i<dirs;i++){
      res=lcd.SDFgetDirName(i,buffer); //get Dir number i name in buffer
      error(res);
      lcd.string(10,row,LCD_WIDTH-1,LCD_HEIGHT-1,buffer,&nb); row+=12;      //print the name if fit
    }
    
    //print file names
    row+=10;
    lcd.string(10,row,LCD_WIDTH-1,LCD_HEIGHT-1,"File Names--------------------",&nb);             row+=12;
    for(i=0;i<files;i++){
