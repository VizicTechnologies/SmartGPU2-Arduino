...
void loop() { //main loop 
    ...
    lcd.string(10,row,LCD_WIDTH-1,LCD_HEIGHT-1,"EEPROM page open, read, write demo!",&nb);   row+=30;
    lcd.string(10,row,LCD_WIDTH-1,LCD_HEIGHT-1,"Performing procedure...",&nb);               row+=30;
    ...
    if(result == 1){  //if contents are equal - means that data was already written and its equal
        lcd.string(10,row,LCD_WIDTH-1,LCD_HEIGHT-1,"EEPROM PAGE0 Contents were already equal...END",&nb);
      ...
      if(result == 1){ //if contents are equal
        lcd.string(10,row,LCD_WIDTH-1,LCD_HEIGHT-1,"New EEPROM PAGE0 Contents are equal...END",&nb);
      }else{           //error
        lcd.string(10,row,LCD_WIDTH-1,LCD_HEIGHT-1,"New EEPROM PAGE0 Contents differ...END",&nb);
      }        
    }
    //loop forever
    while(1);
