...
void loop() { //main loop
  ...
  while(1){
    ...
    lcd.string(10,10,300,220,"This is the string test \nfor SMARTGPU2 processor",&nb);               //write a string on the screen
    delay(2000);    
    //config strings
    lcd.setTextColour(SG_GREEN);
    lcd.setTextSize(SG_FONT3);
    lcd.string(10,80,300,220,message,&nb);                                                           //write the string previously created (char message[]="Hello World";)
    delay(1000);
    //config strings
    lcd.setTextColour(SG_MAGENTA);    
    lcd.putLetter(100,150,'E',&nb);                                                                  //write a single letter 'E'
    delay(1000);
    ...
    delay(1000);    
    lcd.stringSD(5,50,300,230,SG_BEGINNING,SG_ALLCONTENTS,"text file 1",&nb);                        //call the text file "text file 1.txt" previously stored on the micro SD card
    delay(2000);   
